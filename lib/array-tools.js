...
ArrayTools.extract = extract //extract from an array of elements where the query is satisfied
...
ArrayTools.createField = createField
...
;
['shuffle', 'pluck', 'pick', 'pickNull', 'pickFirst', 'arrayify', 'where', 'without', 'uniqueValues', 'uniqueObjects', 'spliceWhile', 'extract', 'flatten', 'sortBy', 'intersect', 'removeFields', 'renameFields', 'formatFieldValues', 'onlyKeepFields', 'values', 'groupFields'].forEach(function(method) {
  ArrayTools.prototype[method] = function() {
...
function groupByAggregate(arr, fields, aggregateFunctions, extraFieldDictionary, onlyKeepDescribedFields) {
  ...
  for (let aggregateKey in rDictionary) {
    ...
    if (toAggregatesArray.length === 1) {
      ...
      var aggregateTuple = this.onlyKeepFields(toAggregatesArray[0], fields);
      let pluckedTuples = {};
...
function createField(array,fieldName,fieldCreationFunction,elements,useOriginalDictionaryArray) {
  let b = useOriginalDictionaryArray ? array : deepClone(array);
  for (let i in b) {
    let values = [];
    for (let j in elements) {
      let element = elements[j];
      if (b[i].hasOwnProperty(element)) {
...
function createFields(array,fieldArrayDictionary,useOriginalDictionaryArray) {
  ...
  for (let i in fieldArrayDictionary) {
    ...
    createField(b,fieldDictionary.fieldName,fieldDictionary.fieldCreationFunction,fieldDictionary.elements,true);
  }
  return b;
}