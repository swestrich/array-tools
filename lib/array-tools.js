...
;
['shuffle', 'pluck', 'pick', 'pickNull', 'pickFirst', 'arrayify', 'where', 'without', 'unique', 'uniqueObjects', 'spliceWhile', 'extract', 'flatten', 'sortBy', 'intersect', 'removeFields', 'renameFields', 'formatFieldValues', 'keepFields', 'merge', 'values', 'groupFields'].forEach(function(method) {
  ArrayTools.prototype[method] = function() {
...
function onlyKeepFields(a, fields,useOriginalDictionaryArray) {
  ...
  for (var i in b) {
    ...
    var removeFields = a.minus(allFields,fields);
...
function createPoints(a, measurement,fieldNames,tagDictionary,timestampField) {
  var rValues = [];
  for (var i in a) {
    var fields = this.keepFields(a[i],fieldNames);
